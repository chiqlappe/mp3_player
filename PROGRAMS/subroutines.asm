...
SYS_PRINT		equ 	$52ED	;HL以降に格納された文字列(0終端)を出力する
	...
	JP	DFP.USR_RESET		;+00 初期化ルーチン。最初に一度だけUSR関数でコールする。エラーなら戻り値=-1
	JP	DFP.USR_CMD		;+03 コマンド送信ルーチン。コマンドとパラメータをメモリにPOKEしてからUSR関数でコールする。エラーなら戻り値=-1
...
.ERROR:	LD	A,$FF			;エラーなら-1を戻り値とする
	JR	.EXIT

.GOOD:	LD	A,0			;正常なら0を戻り値とする
.EXIT:	POP	HL			;=ポインタ
	LD	(HL),A			;戻り値をセットする
	INC	HL
	LD	(HL),A
	...
	;USR関数用
	;コマンド送信
	; in:A=引数の型,HL=ポインタ,(DFP.PARAM)=パラメータ上位,(DFP.PARAM+1)=パラメータ下位,(DFP.CMD)=コマンド番号
.USR_CMD:
	CP	2
